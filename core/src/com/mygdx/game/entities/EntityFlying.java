 ...
 * @author Fery
 */
public abstract class EntityFlying extends Entity{

    public EntityFlying(Builder b){
        super(b);
        if (heightInBlocks == 0)
            heightInBlocks = 1.2f;
        if (entityTextureSizeRatio == 0)
            entityTextureSizeRatio = 87f/205f;
        setSize();
        defineBody(b.getX(), b.getY());
        
        timeMove = (int )(Math.random() * 500) + 200;
        timeToState = (int )(Math.random() * 20) + 100;
    }
